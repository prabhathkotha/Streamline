...
export async function requestMovieData() {
    for(let i in platforms) {   // generate report card for each platform
        ...
        let movieInfo = await getMediaInfo('movie', platforms[i]);
        let showInfo = await getMediaInfo('show', platforms[i]);
        // let showInfo = await getShowInfo(platforms[i]);

        reportCards[platforms[i]].movies = movieInfo;
        reportCards[platforms[i]].shows = showInfo;
    }    
};
